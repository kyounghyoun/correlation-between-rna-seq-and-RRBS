...
def myFileIndexingCheck(REF,mRNA):
    if not os.access(mRNA+".bwt", os.F_OK):
        print "Please index a Reference file ( bwa index %s )" % mRNA
        sys.exit()

    if not os.access(REF.replace(".fa","")+".rev.2.bt2", os.F_OK):
        print "Please index by (bowtie2-build Reference.fa Reference) and make a symbolic links (ln -s Reference.fa Reference)" % REF
        sys.exit()
...
def myJobExecuteAndSuspending(SCRIPTS,Distributable=True):
    def myJob(script):
        print "Script: %s is running [%s]" % (script, myTime())
